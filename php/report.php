...
try {
    ...
    // 必須チェック
    if (empty($reason)) {
        echo json_encode(["success" => false, "error" => "通報理由は必須です"], JSON_UNESCAPED_UNICODE);
        exit;
    }
    if (empty($details)) {
    ...
    // SQL準備
    $stmt = $mysqli->prepare(
        "INSERT INTO reports (post_id, email, reason, details, created_at) VALUES (?, ?, ?, ?, NOW())"
    );
    if (!$stmt) {
    ...
    // バインド
    $stmt->bind_param("ssss", $post_id, $email, $reason, $details);
